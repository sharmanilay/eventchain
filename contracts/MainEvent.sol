...
contract MainEvent is Ownable, ReentrancyGuard, ERC721 {
  ...
  function createEvent(string memory title, string memory description, string memory imageUrl, uint256 capacity, uint256 ticketCost, uint256 startsAt, uint256 endsAt) public {
    ...
    require(capacity > 0, 'Capacity must be greated than zero');
  ...
  function updateEvent(uint256 eventId,string memory title,string memory description,string memory imageUrl,uint256 capacity,uint256 ticketCost,uint256 startsAt,uint256 endsAt
  ) public {
  ...
  function buyTickets(uint256 eventId, uint256 numOfTicket) public payable {
    require(eventExists[eventId], 'Event not found');
    require(msg.value >= events[eventId].ticketCost * numOfTicket, 'Insufficieint amount');
    require(numOfTicket > 0, 'Number of Tickets must be greater than zero');
