...
async function main() {
  ...
  try {
    ...
    const { eventChainContract: eventChainAddress } = JSON.parse(contractAddresses)

    eventChainContract = await ethers.getContractAt('EventChain', eventChainAddress)

    // Process #1
    await Promise.all(
      generateEventData(dataCount).map(async (event) => {
        await createEvent(eventChainContract, event)
      })
    )
        ...
        .map(async (_, i) => {
          ...
          await Promise.all(
            Array(randomCount)
              .fill()
              .map(async () => {
                await buyTickets(eventChainContract, i + 1, 1)
              })
          )
        })
    )
    ...
    // Process #3
    await getEvents(eventChainContract)
    await getTickets(eventChainContract, 1)
  } catch (error) {
